...
router.put('/commandes/:id', async (req, res) => {
  try {
    ...
    const champs = ['client_id', 'fauteuil_id', 'annee_onglet', 'groupe', 'distributeur_nom', 'modele', 'quantite', 'accessoire',
      ...
      'reliquat_suivi', 'reliquat_transporteur', 'reliquat_bl', 'reliquat_date', 'modele_demo',
      'num_retour', 'transporteur_retour', 'date_retour', 'num_commande_distrib',
...
// Bon de livraison du reliquat (PDF ou photo) — stocké en base64 (Render-safe).
router.post('/commandes/:id/reliquat-bl', uploadPreuveLivraison.single('fichier'), async (req, res) => {
  try {
    const cmd = await db.get('SELECT id FROM commandes WHERE id=$1', [req.params.id]);
    if (!cmd) return res.status(404).json({ error: 'Commande introuvable' });
    if (!req.file) return res.status(400).json({ error: 'Aucun fichier reçu' });
    const base64data = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
    const row = await db.run(
      `UPDATE commandes SET reliquat_bl_filename=$1, reliquat_bl_mime=$2, reliquat_bl_data=$3,
        reliquat_bl_uploaded_at=NOW(), updated_at=NOW() WHERE id=$4 RETURNING id, reliquat_bl_filename, reliquat_bl_mime, reliquat_bl_data`,
      [req.file.originalname || 'BL', req.file.mimetype, base64data, req.params.id]
    );
    res.json(row);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
router.delete('/commandes/:id/reliquat-bl', async (req, res) => {
  try {
    const row = await db.run(
      `UPDATE commandes SET reliquat_bl_filename=NULL, reliquat_bl_mime=NULL, reliquat_bl_data=NULL,
        reliquat_bl_uploaded_at=NULL, updated_at=NOW() WHERE id=$1 RETURNING id`, [req.params.id]);
    if (!row) return res.status(404).json({ error: 'Commande introuvable' });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
